...
import { LOTTERIES, AI_SPECIALISTS, getBrasiliaTime, formatBrasiliaHour, type LotteryConfig } from '@/lib/lotteryConstants';
import { useAuth } from '@/hooks/useAuth';
import { supabase } from '@/integrations/supabase/client';
import { Brain, Zap, Activity, Clock, Target, TrendingUp, Bell, CheckCircle, Loader2 } from 'lucide-react';
...
const DashboardHome = () => {
  ...
  // Refresh AI activities every 4 seconds
  useEffect(() => {
  ...
  useEffect(() => {
    if (!user) return;
    supabase.from('gate_history').select('id', { count: 'exact', head: true }).then(({ count }) => {
      setGatesCount(count || 0);
    });
    supabase.from('bets').select('id', { count: 'exact', head: true }).eq('status', 'confirmed').then(({ count }) => {
      setBetsCount(count || 0);
    });
  }, [user]);

  // Simulated auto-analysis that runs in background
  useEffect(() => {
    const autoAnalysis = setInterval(() => {
      const lottery = LOTTERIES[Math.floor(Math.random() * LOTTERIES.length)];
      const confidence = 98 + Math.random() * 2;
      if (confidence >= 100) {
        const msg = `🎯 GATE FOUND — ${lottery.name} — ${confidence.toFixed(3)}%`;
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          ...
          <p className="text-muted-foreground text-sm">Sistema DommoSupremo — {AI_SPECIALISTS.length} IAs Ativas — Auto-Análise 24/7</p>
        </div>
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-2 px-3 py-2 rounded-lg bg-success/10 border border-success/30">
            ...
            <span className="text-xs font-display font-bold text-success tracking-wider">ONLINE</span>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          ...
          { label: 'Taxa de Confiança', value: '100%', icon: TrendingUp, color: 'text-warning' },
        ].map((stat, i) => (
          <motion.div
            key={stat.label}
      ...
      <div className="glass rounded-xl p-5">
        <div className="flex items-center justify-between mb-4">
          ...
          <span className="text-xs px-2 py-1 rounded-full bg-success/10 text-success font-mono animate-pulse">
            ● LIVE
          </span>
        </div>
          ...
          {activities.map((act, i) => (
            ...
            >
              <div className="flex items-center justify-between mb-1.5">
                ...
                <span className={`text-[10px] font-display font-bold tracking-wider ${statusColors[act.status]}`}>
